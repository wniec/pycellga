...
def test_maxcut100(maxcut_instance):
    chromosome = [0] * 100
    expected_result = 0.0
    assert maxcut_instance.f(chromosome) == expected_result, (
        f"Beklenen: {expected_result}, Bulunan: {maxcut_instance.f(chromosome)}"
    )
    ...
    assert computed_result == expected_result, (
        f"Beklenen: {expected_result}, Bulunan: {computed_result}"
    )
